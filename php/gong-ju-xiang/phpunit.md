...
在这篇文章中，我们使用`composer`的依赖包管理工具进行`phpunit`包安装和管理，composer 官方地址[https://getcomposer.org/](https://getcomposer.org/)，按照提示进行全局安装即可，另外，我们也会使用一个非常好用的[Monolog](https://github.com/Seldaek/monolog)记录日志组件记录日志，方便我们查看。
...
## 三、安装PHPUnit {#articleHeader2}

使用`composer`方式安装 PHPUnit，其他安装方式[请看这里](http://www.phpunit.cn/manual/current/zh_cn/installation.html)

```
composer
require
--dev phpunit/phpunit ^6.2
```
...
### 3、高级用法 {#articleHeader6}

你是否已经厌烦了在每一个测试方法命名前面加一个test，是否因为只是调用的参数不同，却要写多个测试用例而纠结？我最喜欢的高级功能，现在隆重推荐给你，叫做`框架生成器`。
...
其他用法请参考官网：[PHPUnit中国官网](http://www.phpunit.cn/)
