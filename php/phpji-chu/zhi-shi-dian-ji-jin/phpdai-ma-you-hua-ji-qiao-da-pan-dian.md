...
27. > 1. 尽量做缓存，可使用memcached。memcached是一款高性能的内存对象缓存系统，可用来加速动态Web应用程序，减轻数据库负载。对运算码 \(OP code\)的缓存很有用，使得脚本不必为每个请求做重新编译。
28. 如果能将**类的方法定义成static**，就尽量定义成static，它的**速度会提升将近4倍**。

29. $row\[’id’\] 的速度是$row\[id\]的7倍。

30. 注销那些不用的变量尤其是大数组，以便释放内存。

31. 尽量避免使用\_\_get，\_\_set，\_\_autoload。

32. require\_once\(\)代价昂贵。

33. 数据库连接当使用完毕时应关掉，不要用长连接。

34. 错误消息代价昂贵。

35. 评估检验\(profile\)你的代码。检验器会告诉你，代码的哪些部分消耗了多少时间。Xdebug调试器包含了检验程序，评估检验总体上可以显示出代码的瓶颈。

36. 在可以用file\_get\_contents替代file、fopen、feof、fgets等系列方法的情况下，**尽量用file\_get\_contents**，因为他的效率高得多！

37. > 1. 但是要注意file\_get\_contents在打开一个URL文件时候的PHP版本问题
38. dsdf 
